...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {   // 此处的sec_cnt为32位大小
   ASSERT(lba <= max_lba);
   ASSERT(sec_cnt > 0);
   sync_acquire (&hd->my_channel->lock);
...
/* 1 先选择操作的硬盘 */
   ...
   /*********************   阻塞自己的时机  ***********************
      ...
      usema_down(&hd->my_channel->disk_done);
   /*************************************************************/
   ...
   sync_release(&hd->my_channel->lock);
}
...
static void partition_scan(struct disk* hd, uint32_t ext_lba) {
   struct boot_sector* bs = kalloc(sizeof(struct boot_sector));
...
void ide_init() {
   ...
   uint8_t hd_cnt = *((uint8_t*)(0xc0000475));	      // 获取硬盘的数量
   ASSERT(hd_cnt > 0);
   ...
   直到硬盘完成后通过发中断,由中断处理程序将此信号量sema_up,唤醒线程. */
      ...
      idt_register(channel->irq_no, intr_hd_handler);
