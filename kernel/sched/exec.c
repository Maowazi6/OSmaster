...
static bool segment_load(int32_t fd, uint32_t offset, uint32_t filesz, uint32_t vaddr) {
	...
	while (page_idx < occupy_pages) {
		...
		if (!(*pde & 0x00000001) || !(*pte & 0x00000001)) {
			if (ualloc_with_vaddr(vaddr_page) == NULL) {
				return false;
			}
...
int32_t sys_execv(const char* path, const char* argv[]) {
	...
	/* 修改进程名 */
	memcpy(cur->proc_name, path, PROCNAME_LIMIT_LEN);
	
	/* 修改栈中参数 */
	struct trap_context* sp = (struct trap_context*)((uint32_t)cur->kstack - sizeof(struct trap_context));
	/* 参数传递给用户进程 */
	sp->ebx = (int32_t)argv;
	sp->ecx = argc;
	sp->eip = (void*)entry_point;
	/* 使新用户进程的栈地址为最高用户空间地址 */
	sp->esp0 = (void*)0xc0000000;
	
	/* exec不同于fork,为使新进程更快被执行,直接从中断返回 */
	asm volatile ("movl %0, %%esp; jmp int_exit" : : "g" (sp) : "memory");
	return 0;
}
