...
/* 文件表 */
struct llfs_file file_table[MAX_FILE_OPEN];

struct file_operations llfs_file_ops = {
	.open = llfs_open,
};
...
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type) {
	...
	part->blk->blk_ops->blk_write(part->blk, sec_lba, 1, bitmap_off);
}
...
int32_t file_create(struct partition *part, struct llfs_dir* parent_dir, char* filename, uint8_t flag) {
	...
	if (io_buf == NULL) {
		printk("in file_creat: kalloc for io_buf failed\n");
		return -1;
	}
	...
	struct llfs_inode* new_file_inode = (struct llfs_inode*)kalloc(sizeof(struct llfs_inode)); 
	if (new_file_inode == NULL) {
		printk("file_create: kalloc for inode failded\n");
		rollback_step = 1;
		goto rollback;
	}
	...
	list_push(&part->open_inodes, &new_file_inode->inode_tag);
...
rollback:
	...
	switch (rollback_step) {
		case 3:
			memset(&file_table[fd_idx], 0, sizeof(struct llfs_file)); 
		case 2:
...
int32_t file_open(struct partition *part, uint32_t inode_no, uint8_t flag) {
	...
	printk("block %d\n",file_table[fd_idx].fd_inode->i_sectors[0]);
...
int32_t file_write(struct partition *part, struct llfs_file* file, const void* buf, uint32_t count) {
	...
	uint8_t* io_buf = kalloc(BLOCK_SIZE);
	if (io_buf == NULL) {
		printk("file_write: kalloc for io_buf failed\n");
		return -1;
	}
	uint32_t* all_blocks = (uint32_t*)kalloc(BLOCK_SIZE + 48);	  // 用来记录文件所有的块地址
	if (all_blocks == NULL) {
		printk("file_write: kalloc for all_blocks failed\n");
		return -1;
	}
 ...
 * 	后面都统一在all_blocks中获取写入扇区地址 */
	...
	/* 在同一扇区内写入数据,不涉及到分配新扇区 */
		if (file_has_used_blocks <= 12 ) {	// 文件数据量将在12块之内
			...
			part->blk->blk_ops->blk_read(part->blk, indirect_block_table, 1, all_blocks + 12);
		}
	} else {
	...
	/* 第一种情况:12个直接块够用*/
		if (file_will_use_blocks <= 12 ) {
			...
			part->blk->blk_ops->blk_write(part->blk, indirect_block_table, 1, all_blocks + 12);      // 同步一级间接块表到硬盘
			...
			part->blk->blk_ops->blk_read(part->blk, indirect_block_table, 1, all_blocks + 12); // 获取所有间接块地址
	...
	while (bytes_written < count) {      // 直到写完所有数据
		...
		if (first_write_block) {
			part->blk->blk_ops->blk_read(part->blk, sec_lba, 1, io_buf);
			first_write_block = false;
		}
		memcpy(io_buf + sec_off_bytes, src, chunk_size);
		part->blk->blk_ops->blk_write(part->blk, sec_lba, 1, io_buf);
...
int32_t file_read(struct partition *part, struct llfs_file* file, void* buf, uint32_t count) {
	...
	uint8_t* io_buf = kalloc(BLOCK_SIZE);
	if (io_buf == NULL) {
		printk("file_read: kalloc for io_buf failed\n");
	}
	uint32_t* all_blocks = (uint32_t*)kalloc(BLOCK_SIZE + 48);	  // 用来记录文件所有的块地址
	if (all_blocks == NULL) {
		printk("file_read: kalloc for all_blocks failed\n");
		return -1;
	}
...
/* 	以下开始构建all_blocks块地址数组,专门存储用到的块地址(本程序中块大小同扇区大小) */
	...
	/* 第一种情况: 起始块和终止块属于直接块*/
		...
		block_idx = block_read_start_idx; 
			...
			/* 再将间接块地址写入all_blocks */
			indirect_block_table = file->fd_inode->i_sectors[12];
			part->blk->blk_ops->blk_read(part->blk, indirect_block_table, 1,  all_blocks + 12);	      // 将一级间接块表读进来写入到第13个块的位置之后
		} else {	
			/* 第三种情况: 数据在间接块中*/
			ASSERT(file->fd_inode->i_sectors[12] != 0);	    // 确保已经分配了一级间接块表
			indirect_block_table = file->fd_inode->i_sectors[12];	      // 获取一级间接表地址
