...
struct llfs_dir root_dir;             // 根目录

const struct inode_operations llfs_dir_inode_operations = {
	.lookup = llfs_look_up,
	.mkdir = llfs_mkdir,
	.create = llfs_create,
	.rmdir = llfs_rmdir
...
const struct file_operations llfs_dir_file_operations = {
	.open = llfs_dir_open,
	.read = dir_read,
	.write = NULL
};
...
struct llfs_dir* dir_open(struct partition* part, uint32_t inode_no) {
   struct llfs_dir* pdir = (struct llfs_dir*)kalloc(sizeof(struct llfs_dir));
   pdir->inode = inode_open(part, inode_no);
   pdir->dir_pos = 0;
   return pdir;
}

bool search_dir_entry(struct partition* part, struct llfs_dir* pdir, \
		     const char* name, struct llfs_dir_entry * dir_e) {
	uint32_t block_cnt = 140;	 // 12个直接块+128个一级间接块=140块
	
	/* 12个直接块大小+128个间接块,共560字节 */
	uint32_t* all_blocks = (uint32_t*)kalloc(48 + 512);
	if (all_blocks == NULL) {
		printk("search_dir_entry: kalloc for all_blocks failed");
		return false;
	}
	...
	if (pdir->inode->i_sectors[12] != 0) {	// 若含有一级间接块表
		part->blk->blk_ops->blk_read(part->blk, pdir->inode->i_sectors[12], 1, all_blocks + 12);
	}
	
	/* 写目录项的时候已保证目录项不跨扇区,
		* 这样读目录项时容易处理, 只申请容纳1个扇区的内存 */
	uint8_t* buf = (uint8_t*)kalloc(SECTOR_SIZE);
	struct llfs_dir_entry * p_de = (struct llfs_dir_entry *)buf;	    // p_de为指向目录项的指针,值为buf起始地址
	...
	/* 块地址为0时表示该块中无数据,继续在其它块中找 */
		...
		part->blk->blk_ops->blk_read(part->blk, all_blocks[block_idx], 1, buf);
		...
		block_idx++;
		p_de = (struct llfs_dir_entry *)buf;
...
bool sync_dir_entry(struct partition *part,struct llfs_dir* parent_dir, struct llfs_dir_entry * p_de, void* io_buf) {
	struct llfs_inode* dir_inode = parent_dir->inode;
	...
	struct llfs_dir_entry * dir_e = (struct llfs_dir_entry *)io_buf;	       // dir_e用来在io_buf中遍历目录项
	...
	while (block_idx < 140) { // 文件(包括目录)最大支持12个直接块+128个间接块＝140个块
		...
		if (all_blocks[block_idx] == 0) {  // 在三种情况下分配块
			...
			if (block_idx < 12) {	    // 若是直接块
				...
				block_lba = -1;
				block_lba = block_bitmap_alloc(part);	       // 再分配一个块做为第0个间接块
				if (block_lba == -1) {
			...
			/* 每分配一个块就同步一次block_bitmap */
				...
				part->blk->blk_ops->blk_write(part->blk, dir_inode->i_sectors[12], 1, all_blocks + 12);
			} else {	   // 若是间接块未分配
			...
			part->blk->blk_ops->blk_write(part->blk, all_blocks[block_idx], 1, io_buf);
			dir_inode->i_size += dir_entry_size;
			return true;
		}
	
		/* 若第block_idx块已存在,将其读进内存,然后在该块中查找空目录项 */
		part->blk->blk_ops->blk_read(part->blk, all_blocks[block_idx], 1, io_buf); 
		/* 在扇区内查找空目录项 */
...
bool delete_dir_entry(struct partition* part, struct llfs_dir* pdir, uint32_t inode_no, void* io_buf) {
	struct llfs_inode* dir_inode = pdir->inode;
	...
	if (dir_inode->i_sectors[12]) {
		part->blk->blk_ops->blk_read(part->blk, dir_inode->i_sectors[12], 1, all_blocks + 12);
	}
	
	/* 目录项在存储时保证不会跨扇区 */
	uint32_t dir_entry_size = part->sb->dir_entry_size;
	uint32_t dir_entrys_per_sec = (SECTOR_SIZE / dir_entry_size);       // 每扇区最大的目录项数目
	struct llfs_dir_entry * dir_e = (struct llfs_dir_entry *)io_buf;   
	struct llfs_dir_entry * dir_entry_found = NULL;
...
struct llfs_dir_entry * dir_read(struct partition *part, struct llfs_dir* dir) {
	struct llfs_dir_entry * dir_e = (struct llfs_dir_entry *)dir->dir_buf;
	struct llfs_inode* dir_inode = dir->inode; 
	...
	while (block_idx < block_cnt) {
		...
		part->blk->blk_ops->blk_read(part->blk, all_blocks[block_idx], 1, dir_e);
		dir_entry_idx = 0;
		/* 遍历扇区内所有目录项 */
		while (dir_entry_idx < dir_entrys_per_sec) {
...
int32_t dir_remove(struct partition *part, struct llfs_dir* parent_dir, struct llfs_dir* child_dir) {
	struct llfs_inode* child_dir_inode  = child_dir->inode;
	...
	delete_dir_entry(part, parent_dir, child_dir_inode->i_no, io_buf);
	/* 回收inode中i_secotrs中所占用的扇区,并同步inode_bitmap和block_bitmap */
	inode_release(part, child_dir_inode->i_no);
	kfree(io_buf);
	return 0;
}
