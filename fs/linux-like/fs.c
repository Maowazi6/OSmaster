...
static bool mount_partition(struct list_elem* pelem, int arg) {
	...
	struct partition* part = STRUCT_ENTRY(struct partition, part_tag, pelem);
	if (!strcmp(part->name, part_name)) {
		cur_part = part;
		struct disk* hd = cur_part->my_disk;
		/* sb_buf用来存储从硬盘上读入的超级块 */
		struct super_block* sb_buf = (struct super_block*)kalloc(SECTOR_SIZE);
		/* 在内存中创建分区cur_part的超级块 */
		cur_part->sb = (struct super_block*)kalloc(sizeof(struct super_block));
		if (cur_part->sb == NULL) {
		...
		/*将硬盘上的块位图读入到内存*/
		cur_part->block_bitmap.bits = (uint8_t*)kalloc(sb_buf->block_bitmap_sects * SECTOR_SIZE);
		...
		/*将硬盘上的inode位图读入到内存*/
		cur_part->inode_bitmap.bits = (uint8_t*)kalloc(sb_buf->inode_bitmap_sects * SECTOR_SIZE);
...
char* sys_getcwd(char* buf, uint32_t size) {
	...
	ASSERT(buf != NULL);
	void* io_buf = kalloc(SECTOR_SIZE);
	if (io_buf == NULL) {
		return NULL;
	}
	struct proc_struct* cur_thread = get_cur_proc();
...
int32_t sys_chdir(const char* path) {
	...
	if (inode_no != -1) {
		if (searched_record.file_type == FT_DIRECTORY) {
			get_cur_proc()->cwd_inode_nr = inode_no;
			ret = 0;
		} else {
