...
typedef void* (*shell_func)(uint32_t, char**);
extern int32_t argc;
extern char *argv[MAX_ARG_NR];
extern char final_path[MAX_PATH_LEN];
HashTable shell_cmd_htb;

//任务,execv,修修bug,有时间就写vfs,dev,内存再细化一下
...
void make_clear_abs_path(char* path, char* final_path) {
	...
	if (path[0] != '/') {      // 若输入的不是绝对路径,就拼接成绝对路径
		...
		if (getpwd(abs_path, MAX_PATH_LEN) != NULL) {
...
void buildin_pwd(uint32_t argc, char** argv UNUSED) {
	if (argc != 1) {
		...
		if (NULL != getpwd(final_path, MAX_PATH_LEN)) {
			printf("%s\n", final_path); 
		} else {
...
//		struct llfs_dir* dir = opendir(pathname);
//		struct llfs_dir_entry* dir_e = NULL;
...
void buildin_cmd_init(){
	HTBInit(&shell_cmd_htb, 50);
	//register_buildin_cmd("ls",buildin_ls);
	//register_buildin_cmd("rm",buildin_rm);
	register_buildin_cmd("rmdir",buildin_rmdir);
	register_buildin_cmd("mkdir",buildin_mkdir);
	register_buildin_cmd("clear",buildin_clear);
	register_buildin_cmd("cd",buildin_cd);
	register_buildin_cmd("pwd",buildin_pwd);
	register_buildin_cmd("ps",buildin_ps);
	register_buildin_cmd("help",buildin_help);
}

void register_buildin_cmd(char *str, shell_func func){
	str_hash_insert(&shell_cmd_htb, str,func);
}

uint32_t buildin_cmd_exec(char *str){
	HashNode *node = str_hash_find(&shell_cmd_htb,str);
	if(node!= NULL){
